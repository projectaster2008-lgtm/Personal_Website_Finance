...
export async function loadEntries(
  ...
  return rows.map((row) => ({
    ...
    // Defensive: an entry whose transaction has vanished is a bug, not a normal
    // state. We surface it in the UI instead of throwing, so one bad row cannot
    // take down every report at once.
    transactionType: row.transaction?.type ?? 'EXPENSE',
    description: row.transaction?.description ?? '(orphaned entry - please report)',
  }));
}
